...
function open_upload(username) {
	window.open('upload_document.php?user=' + username,'uploadWnd',"status=0,toolbar=0,location=0,menubar=0,directories=0,resizable=0,scrollbars=0,height=200,width=400");		
}

function download_document(filePath) {
	window.open('download_document.php?file=' + filePath,'downloadWnd');
}
...
<div align="center">
<?php
	echo "<h1>$manDocumentsTitle</h1>\n";
	echo "<h3 align=\"left\">[ <a href=\"#\" onclick=\"open_upload('$username')\">$uploadDocumentMsg</a> ]</h3>";
?>  
  <table width="200" border="1">
    <tr>
<?php
	  echo "<th></th>";
      echo "<th><div align=\"center\">$programCap[2]</div></th>";
?>
      <th colspan="4"><div align="center"></div></th>
    </tr>
    
<?php
	$url = "$host/axis2/services/ws_ems/get_documents_from_user?username=$username&cookie=$cookie_value&ip=$ip";
	//echo $url;
	$myfile = fopen($url, "r")
			or die("died when doing OPEN ws_ems.get_documents_from_user");
	$allLines = "";
	...
	for($i = 0; $i < $cnt ; $i++) {
		...
		if ($xmlNode !== "false") {
			...
			if ($i % 2 === 0)
				...
				echo "<tr>";				
			if ($line[2] === "pdf") {		
				echo "<td><div align=\"center\"><a href=\"download_document.php?user=$username&fid=$line[1]&ext=$line[2]&desc=$line[3]\"><img src=\"img/pdf.png\" width=\"100\" height=\"100\" /></a></div></td>";			
			} else		
				echo "<td><div align=\"center\"><a href=\"download_document.php?user=$username&fid=$line[1]&ext=$line[2]&desc=$line[3]\"><img src=\"photo.php?user=$username&fid=$line[1]\" width=\"100\" height=\"100\" /></a></div></td>";
			//echo "<td><div align=\"center\"><a href=\"download_document.php?file=$filePath\"><img src=\"$filePath\" width=\"100\" height=\"100\" /></a></div></td>";
			echo "<td><div align=\"center\">$line[3]</div></td>";
			echo "<td><div align=\"center\"><a href=\"#\" onclick=\"delete_document('$username','$line[1]')\">$delMsg</a></div></td>";
			echo "</tr>";
		}		
