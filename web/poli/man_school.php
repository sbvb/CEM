...
<?php
echo "<h1 align=\"center\">$manSchoolTitle</h1>\n";
	echo "<h3>[ <a href=\"crt_school.php?user=$username&fc=crt\">$crtSchoolMsg</a> ]</h3>";
	
	$url = "$host/axis2/services/ws_ems/get_school_list?username=$username&cookie=$cookie_value&ip=$ip";
		//echo "url = $url<br>";
		$myfile = fopen($url, "r")
        		or die("died when doing OPEN get_school_list");
		$allLines = "";
		...
		for($i = 0; $i < $cnt ; $i++) {
			...
			if ($i == 0) {
				...
				if ($i % 2 === 0)
					...
					echo "\t<tr>\n";
				echo '<td>' . $line[0] . '</td>';
				echo "<td><a href=\"crt_school.php?user=$username&fc=edt&edtusr=$line[1]\">$editMsg</a></td>";
				echo "<td><a href=\"#\" onClick=\"delete_school('$username','$line[1]')\">$delMsg</a></td>";
				echo "\t</tr>\n";
			}
		}
	echo "</table>\n";
