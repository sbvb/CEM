...
<?php
	echo "<h1 align=\"center\">$shDatetimeMsg</h1>\n";
	
	$url = "$host/axis2/services/ws_ems/get_datetime_settings?username=$username&cookie=$cookie_value&ip=$ip&dtid=$dtid";
	$myfile = fopen($url, "r")
       		or die("died when doing OPEN ws_ems.get_datetime_settings");
	$allLines = "";
	...
	$cnt = $xmlElem->length;
	echo "<table border =\"1\" align=\"center\" style=\"background-color:yellow\">\n";
	for($i = 0; $i < $cnt ; $i++) {
		...
		echo "\t<tr>\n";
		echo "<td>$datetimeCap[0]:</td>";
		echo '<td>' . $line[0] . '</td>';
		echo "\t</tr>\n";
		echo "\t<tr>\n";
		echo "<td>$datetimeCap[1]:</td>";
		echo '<td>' . $line[1] . '</td>';
		echo "\t</tr>\n";
		echo "\t<tr>\n";
		echo "<td>$datetimeCap[2]:</td>";
		echo '<td>' . $line[2] . '</td>';
		echo "\t</tr>\n";
		echo "\t<tr>\n";
		echo "<td>$datetimeCap[3]:</td>";
		echo '<td>' . $line[3] . '</td>';
		echo "\t</tr>\n";								
	}
	echo "</table>\n";
