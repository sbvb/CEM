...
<?php
	echo "<h1 align=\"center\">$manClassTitle</h1>\n";
	echo "<p><h3>[ <a href=\"crt_class_act.php?user=$username&fc=crt\">$crtClassMsg</a> ] </h3></p>";
	
	$url = "$host/axis2/services/ws_ems/get_classes_list?username=$username&cookie=$cookie_value&ip=$ip";
		//echo "url = $url<br>";
		$myfile = fopen($url, "r")
        		or die("died when doing OPEN ws_ems.get_classes_list");
		$allLines = "";
		...
		if ($xmlNode == "session_expired")
			...
			if ($i == 0) {
				...
				if ($i % 2 === 0)
					...
					echo "\t<tr>\n";
				echo '<td>' . $line[0] . '</td>';
				//echo '<td><a href="crt_class_act.php?user=' . $username . '&fc=edt&edusr=' . $line[1] . '">edit</a></td>';
				echo "<td><a href=\"#\" onClick=\"delete_class('$username','$line[1]')\">$delMsg</a></td>\n";
				echo "\t</tr>\n";				
			}
		}
	echo "</table>\n";	
