...
<?php
	echo "<h1 align=\"center\">$manAmbienceTitle</h1>\n";
	echo "<p><h3>[ <a href=\"crt_ambience_act.php?user=$username&fc=crt\">$crtAmbMsg</a> ]</h3></p>";
	...
	$url = "$host/axis2/services/ws_ems/get_ambiences_list?username=$username&cookie=$cookie_value&ip=$ip";
		...
		for($i = 0; $i < $cnt ; $i++) {
			...
			if ($i == 0) {
				...
				echo '<td>' . $line[0] . '</td>';
				echo '<td>' . $line[2] . '</td>';
				echo "<td><a href=\"crt_ambience_act.php?user=$username&fc=edt&edusr=$line[1]\">$editMsg</a></td>";
				echo "<td><a href=\"#\" onClick=\"delete_ambience('$username','$line[1]')\">$delMsg</a></td>\n";
				echo "\t</tr>\n";				
			}
		}
	echo "</table>\n";
