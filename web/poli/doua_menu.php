...
<?php
	$url = "$host/axis2/services/ws_doua_ems/get_resources?username=$username&cookie=$cookie_value&ip=$ip";
		//echo $url;
		$myfile = fopen($url, "r")
		...
		// Changes the home page according with the actor
		if ($actor === "Interested") {
			//header("location: edit_interested.php?user=$username");
			echo "<script>back_page('edit_interested.php?user=$username')</script>";
		} 
		else if ($actor === "Secretary") {
			echo "<script>back_page('admin_man_interested.php?user=$username')</script>";
		}
?>
...
<!-- Fim tabela menu lateral e conteúdo --></td>
</tr>
