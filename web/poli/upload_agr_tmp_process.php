...
$filenameLstCnt = count($filenameLst);
if ($filenameLst[$filenameLstCnt - 1] === "tex") { // latex template upload
...
} else { // uploading images to be used with the templates

	$webservFilename = $newDescription . "." . $filenameLst[$filenameLstCnt - 1];
	$url = "$host/axis2/services/ws_ems/check_agreement_template_image_existence?username=$username&cookie=$cookie_value&ip=$ip&filename=$webservFilename";
	//echo "url = $url<br>";
	$myfile = fopen($url, "r")
		or die("died when doing OPEN ws_ems.check_agreement_template_image_existence");
	$allLines = "";
	while ($currline = fread($myfile,1024))
		$allLines .= $currline; // concat currline to allLines
	fclose($myfile) or die("died when doing CLOSE");
	...
	if ($xmlNode === "false") {
		$uploadFilename = $uploadDirectory. "templates/" . $newDescription . "." . $filenameLst[$filenameLstCnt - 1]; 
		@move_uploaded_file($_FILES[$fieldname]['tmp_name'], $uploadFilename)
			or header("location: upload_error.php?error=" . $_FILES[$fieldname]['error']);
		...
		if (!$fp) {
			...
			while (!feof($fp)) {  
				...
				$url = "$host/axis2/services/ws_ems/upload_agreement_template_image?filename=$webservFilename&fdata=$newFileData";
				//echo "url = $url<br>";
				$myfile = fopen($url, "r")
					or die("died when doing OPEN ws_ems.upload_file");
				$allLines = "";
		...
		header("location: upload_success.php?filename=" . $_FILES[$fieldname]['name']);
	} else if ($xmlNode === "true") { // file exists
		header("location: upload_error.php?error=36");
	} else if ($xmlNode === "file_not_allowed") { // file not allowed
		header("location: upload_error.php?error=37");
	} else {
		header("location: upload_error.php?error=33");
	}
}
