...
<?php
	echo "<h1 align=\"center\">$manScheduleTitle</h1>\n";
	echo "<p><h3>[ <a href=\"crt_schedule_act.php?user=$username&fc=crt\">$crtScheduleMsg</a> ]</h3></p>";
	
	$url = "$host/axis2/services/ws_ems/get_schedules_list?username=$username&cookie=$cookie_value&ip=$ip";
		//echo "url = $url<br>";
		$myfile = fopen($url, "r")
        		or die("died when doing OPEN ws_ems.get_schedules_list");
		$allLines = "";
		...
		for($i = 0; $i < $cnt ; $i++) {
			...
			if ($i == 0) {
				...
				if ($i % 2 === 0)
					...
					echo "\t<tr>\n";
				echo '<td>' . $line[0] . '</td>';
				//echo "<td><a href=\"show_schedule.php?user=$username&schedule=$line[1]\" target=\"_blank\">view</a></td>\n";
				echo "<td><a href=\"crt_schedule_act.php?user=$username&fc=edt&edusr=$line[1]\">$editMsg</a></td>\n";
				echo "<td><a href=\"#\" onClick=\"delete_schedule('$username','$line[1]')\">$delMsg</a></td>\n";
				echo "\t</tr>\n";				
			}
		}
	echo "</table>\n";
