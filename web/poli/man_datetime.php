...
<?php
	echo "<h1 align=\"center\">$manDatetimeTitle</h1>\n";
	echo "<p><h3>[ <a href=\"crt_datetime_act.php?user=$username&fc=crt\">$crtDatetimeMsg</a> ]</h3></p>";
	
	$url = "$host/axis2/services/ws_ems/get_datetimes_list?username=$username&cookie=$cookie_value&ip=$ip";
		//echo "url = $url<br>";
		$myfile = fopen($url, "r")
        		or die("died when doing OPEN ws_ems.get_datetimes_list");
		$allLines = "";
		...
		for($i = 0; $i < $cnt ; $i++) {
			...
			if ($i == 0) {
				...
				if ($i % 2 === 0)
					...
					echo "\t<tr>\n";
				echo '<td>' . $line[0] . '</td>';
				echo "<td><a href=\"show_datetime.php?user=$username&dtid=$line[1]\" target=\"_blank\">$viewMsg</a></td>\n";
				echo "<td><a href=\"#\" onClick=\"delete_datetime('$username','$line[1]')\">$delMsg</a></td>\n";
				echo "\t</tr>\n";				
			}
		}
	echo "</table>\n";
