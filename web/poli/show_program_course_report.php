...
//$fct		= $_REQUEST['fc'];
$signature	= 'show_program_course_report.php';
...
<?php
	echo "<h1 align=\"center\">$programCourseReportTitle</h1>\n";
				
	echo '<table width="291" align="center" border="1">';				
				
			//Getting the presence report from webservices
			$url = "$host/axis2/services/ws_ems/show_program_course_report?username=$username&cookie=$cookie_value&ip=$ip";
			//echo "url = $url<br>";
			$myfile = fopen($url, "r")
					or die("died when doing OPEN ws_ems.show_program_course_report");
			$allLines = "";
			...
			for($i = 0; $i < $cnt ; $i++) {
				...
				$program = $line[0];
				$coursesLst = explode(":", $line[1]);
				$cntCourses = count($coursesLst);
				
				$programLine = explode(",", $program);
				echo "<tr>\n";
				
				if ($cntCourses > 1)
					echo "<td rowspan=\"$cntCourses\">\n";
				else
					echo "<td>\n";
					
				echo "<strong>$programLine[0]<strong>";
				echo "</td>";
								
				for($j = 0; $j < $cntCourses; $j++) {
					$courseLine = explode(",", $coursesLst[$j]);
					echo "<td>$courseLine[0]</td></tr>";
				}
