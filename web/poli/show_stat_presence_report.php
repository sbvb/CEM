...
//$fct		= $_REQUEST['fc'];
$signature	= 'stat_presence_report.php';

$coline 	= $_REQUEST['lstCourse']; 
$clline 	= $_REQUEST['lstClass'];
$schline = $_REQUEST['lstSchedule'];
...
$schLst			= explode(":", $schline);
$scheduleName	= $schLst[0];
$scheduleId		= $schLst[1];
...
<?php
	echo "<h1 align=\"center\">$statPresenceReportTitle</h1>\n";
	echo "<h3 align=\"center\">($assocClassCourseCap[0] \"$courseName\", $assocClassCourseCap[2] \"$className\")</h3>\n";
		
	//Getting the statistical presence report from webservices
	$max_datetimes = 0;
	$url = "$host/axis2/services/ws_ems/show_statistical_presence_report?username=$username&cookie=$cookie_value&ip=$ip&coid=$courseId&clid=$classId&schid=$scheduleId";
	//echo "url = $url<br>";
	$myfile = fopen($url, "r")
			or die("died when doing OPEN ws_ems.show_statistical_presence_report");
	$allLines = "";
	...
	for($i = 0; $i < $cnt ; $i++) {
		...
		//echo $xmlNode;
		$max_datetimes = $line[0];
		$presenceList[] = $line[1];
	}			
	
	echo "<p align=\"center\">$totalClassesMsg: $max_datetimes</p>\n";
	
	echo '<table width="291" align="center" border="1">';				
		echo "<tr>\n";				
			echo "<th>$studentCap</th>";
			echo "<th>$statPresenceCap</th>";
		echo "</tr>\n";
		
			$preLstCnt = count($presenceList);
			for($i = 0; $i < $preLstCnt ; $i++) {								
				$line = explode(',',$presenceList[$i]);
				//echo $xmlNode;
				if ($i % 2 === 0)
