...
<h4>
<?php
	echo "<h1 align=\"center\">$manPersonTitle</h1>\n";
	
	echo "<h3> [ <a href=\"crt_person_act.php?user=$username&fc=crt\">$crtNewPersonMsg</a> ] </p>";
	
	echo "<h3 align=\"center\">Buscar $actorMsg <select name=\"lstUserGroup\" onchange=\"getGroupId(this, '$username')\">\n";
	...
	if (isset($gline)) { // gets all persons with this group id
		...
		if ($isUserSchoolMan == true) { // username is school manager
			...
			$url = "$host/axis2/services/ws_ems/get_person_list?username=$username&cookie=$cookie_value&ip=$ip&ptype=$gname&sort=name";
			//echo "url = $url<br>";
			$myfile = fopen($url, "r")
