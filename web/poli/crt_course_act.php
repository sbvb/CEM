...
<?php
	if ($fct == $fct_edit)
	{
		$url = "$host/axis2/services/ws_ems/get_course_settings?username=$username&cookie=$cookie_value&ip=$ip&cid=$edt_usr";
		//echo $url;
		$myfile = fopen($url, "r")
				or die("died when doing OPEN ws_doua_ems.get_course_settings");
		$allLines = "";
		...
		$course_settings = $xmlNode; 
		
		$cname	= "";
		$code	= "";						
		if ($course_settings != "") {
			$lst = explode(":", $course_settings);
			$cname	= $lst[0];
			$code	= $lst[1];			
		}
	
		echo "<h1 align=\"center\">$edtCourseActMsg</h1>\n";
		echo "<p>&nbsp; </p>\n";
		echo "<p>$curCourseMsg $cname</p>\n";
		...
		echo '<form action="" method="get" name="frmEdit" id="frmEdit">';
			echo '<table width="291" align="center" style="background-color:yellow">';				
				echo "<tr>\n";
					echo "<td><div align=\"right\">$courseCap[1]:</div></td>";
					echo "<td colspan=\"2\"><input name=\"txtCode\" type=\"text\" id=\"txtCode\" value=\"$code\"></td>\n";
				echo "</tr>\n";				
				echo "<tr>\n";
					echo "<td><input name=\"btnBack\" type=\"button\" value=\"$backBtnMsg\" onClick=\"back_page('man_course.php?user=$username&fc=0')\"></td>";
					echo "<td><input name=\"btnSave\" type=\"button\" value=\"$saveBtnMsg\" onClick=\"edit_course()\"></td>";
				echo "</tr>\n";
			echo "</table>\n";
	...
	{		
		echo "<h1 align=\"center\">$crtCourseActMsg</h1>";
		echo '<p><div id="edt_err_msg" align="center" style="color:#FF0000"></div></p>';
		echo '<form action="" method="get" name="frmCreate" id="frmCreate">';
			echo '<table width="353" align="center" style="background-color:yellow">';
				echo "<tr>\n";
					...
					echo '<td colspan="2"><input name="txtCourse" type="text" id="txtCourse"></td>';
				echo "</tr>\n";
				echo "<tr>\n";
					echo "<td><div align=\"right\">$courseCap[1]:</div></td>";
					echo '<td colspan="2"><input name="txtCode" type="text" id="txtCode"></td>';
				echo "</tr>\n";				
				echo "<tr>\n";
					echo "<td><input name=\"btnBack\" type=\"button\" id=\"btnBack\" value=\"$backBtnMsg\" onClick=\"back_page('man_course.php?user=$username&fc=0')\"></td>";
					echo "<td><input name=\"btnClear\" type=\"reset\" id=\"btnClear\" value=\"$clearBtnMsg\"></td>";
					echo "<td><input name=\"btnCreate\" type=\"button\" id=\"btnCreate\" value=\"$createBtnMsg\" onClick=\"create_course()\"></td>";
				echo "</tr>\n";
			echo "</table>\n";
			echo "<input type=\"hidden\" name=\"hdOwner\" value=\"$username\">\n";
		echo "</form>\n";		
