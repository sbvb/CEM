...
<?php
	echo "<h1 align=\"center\">$answerPresenceTitle</h1><br>\n";
	echo "<h3 align=\"center\">($assocClassCourseCap[0] \"$courseName\", $assocClassCourseCap[2] \"$className\")</h3>\n";
	echo "<h3 align=\"center\">($datetimeCap[0] \"$date\", $timeCap \"$time\")</h3>\n";
	...
	echo '<p><div id="edt_err_msg" align="center" style="color:#FF0000"></div></p>';
		echo '<form action="" method="get" name="frmAssociate" id="frmCreate">';
			echo '<table width="400" align="center" border="1">';				
				echo "<tr>\n";
					echo "<th></th>";				
					echo "<th width=\"150\">$studentCap</th>";
					echo "<th>$presenceCap</th>";
				echo "</tr>\n";
				
						//Getting the students list from enrollment from webservices
						...
						for($i = 0; $i < $cnt ; $i++) {
							...
							if ($i > 0) {
								if ($i % 2 === 0)
									...
									$url2 = "$host/axis2/services/ws_ems/get_photo_from_candidate?username=$username&cookie=$cookie_value&ip=$ip&pid=$line[1]";
									//echo $url;
									$myfile2 = fopen($url2, "r")
											or die("died when doing OPEN ws_ems.get_photo_from_candidate");
									$allLines2 = "";
									while ($currline2 = fread($myfile2,1024))
										$allLines2 .= $currline2; // concat currline to allLines
									fclose($myfile2) or die("died when doing CLOSE");
									// parse XML
									$xmlDoc2 = new DOMDocument();
									$xmlDoc2->loadXML($allLines2);
									
									$xmlElem2 = $xmlDoc2->getElementsByTagName("return"); // not "ns:return"
									$cnt2 = $xmlElem2->length;
									for($j = 0; $j < $cnt2 ; $j++) {
										$xmlNode2 = $xmlElem2->item($j)->nodeValue;	
										
										// Goes to doua_error.php because session expired.
										if ($xmlNode2 == "session_expired")
											echo "<script>back_page('doua_error.php')</script>";
									}
									$photoProperties = explode(":", $xmlNode2);
									...
									if ($photoProperties[0] === "false")
										...
										echo "<td width=\"125\"><img src=\"admin_photo.php?user=$username&pid=$line[1]&fid=$photoProperties[1]\" width=\"125\" height=\"125\"></td>";		
									}
									
									echo "<td align=\"center\">$line[0]</td>";
									echo "<td align=\"center\"><input type=\"checkbox\" name=\"chkPresence\" value=\"$line[1]\" /></td>";
								echo "</tr>\n";
							}						
						}
				...
				echo "<tr>\n";
					echo "<td></td>";
					echo "<td align=\"right\"><input name=\"btnBack\" type=\"button\" value=\"$backBtnMsg\" onClick=\"back_page('answer_presence.php?user=$username')\"></td>";
					echo '<td align="right"><input name="btnOk" type="button" value="OK" onClick="answer_presence()"></td>';				
				echo "</tr>\n";
			echo "</table>\n";			
