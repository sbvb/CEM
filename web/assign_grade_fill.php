...
?>
	...
	<SCRIPT TYPE="text/javascript">
		<!--		
		function validate_grade(what)
		{
			var gradeNum = what.value;
			//alert(gradeNum);
			
			if (isNaN(gradeNum) || ((parseFloat(gradeNum) < 0.0) || (parseFloat(gradeNum) > 10.0))) {
				what.value = "";
				document.getElementById("edt_err_msg").innerHTML = "Grade must be number between 0.0 and 10.0";
				what.focus();
			}
...
<?php 
	...
	if ($isClassCourseFinished == "false") { // tutor can assign grade
	
		//Getting the grades list from webservices
		$url = "$host/axis2/services/ws_ems/show_grade_report?username=$username&cookie=$cookie_value&ip=$ip&clid=$classId&coid=$courseId";
		//echo "url = $url<br>";
		$myfile = fopen($url, "r")
			...
			echo '<form action="" method="get" name="frmAssociate" id="frmCreate">';
				echo '<table width="291" align="center" border="1">';				
					...
					echo "</tr>\n";
							...
							//Getting the students list per enrollment from webservices
							...
							for($i = 0; $i < $cnt ; $i++) {
								...
								if ($i > 0) {
									echo "<tr>\n";
										$iGrade = $i - 1;
										echo "<td>$line[0]</td>";
										echo "<input type=\"hidden\" name=\"hdStudentId\" value=\"$line[1]\" />\n";
										
										
										if ($iGrade < $cntGrade)
											echo "<td><input type=\"text\" name=\"txtGrade\" onchange=\"validate_grade(this)\" value=\"$gradeLst[$iGrade]\" /></td>";
										else
											echo "<td><input type=\"text\" name=\"txtGrade\" onchange=\"validate_grade(this)\" /></td>";								
									echo "</tr>\n";
								}						
							}
										
					echo "<tr>\n";
						echo "<td><input name=\"btnBack\" type=\"button\" value=\"$backBtnMsg\" onClick=\"back_page('assign_grade.php?user=$username')\"></td>";
						echo '<td><input name="btnOk" type="button" value="OK" onClick="assign_grade()"></td>';				
					echo "</tr>\n";
				echo "</table>\n";			
	...
	else { // Grade already assigned
		echo "<h2>$gradeFinishedMsg</h2>";
	}
	echo "</center>\n";
?>
</body>
