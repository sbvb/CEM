...
$signature	= 'man_school.php';

$fct = $_REQUEST['fc'];
$fct_edit	= 'edt';
$fct_create	= 'crt';
if ($fct == $fct_edit) {
	$edt_usr = $_REQUEST['edtusr'];
}
...
?>
	...
	<script type="text/javascript" src="js/ajaxlib.js" charset="utf-8"></script>
...
<?php 
	...
	if ($fct == $fct_create) { // create mode
		
		echo "<h1>$crtSchoolTitle</h1>";
		...
		echo '<form action="" method="get" name="frmCreate" id="frmCreate">';
			echo '<table width="353" align="center">';
				echo "<tr>\n";
					echo "<input type=\"hidden\" name=\"hdUser\" value=\"$username\">\n";
					echo "<td><div align=\"right\">$schoolMsg</div></td>";
					echo '<td colspan="2"><input name="txtSchool" type="text" id="txtSchool"></td>';
				echo "</tr>\n";			
				echo "<tr>\n";
					echo "<td><input name=\"btnBack\" type=\"button\" id=\"btnBack\" value=\"$backBtnMsg\" onClick=\"back_page('man_school.php?user=$username')\"></td>";
					echo "<td><input name=\"btnClear\" type=\"reset\" id=\"btnClear\" value=\"$clearBtnMsg\"></td>";
					echo "<td><input name=\"btnCreate\" type=\"button\" id=\"btnCreate\" value=\"$createBtnMsg\" onClick=\"create_school()\"></td>";
				echo "</tr>\n";
			echo "</table>\n";		
		echo "</form>\n";
		...
		$url = "$host/axis2/services/ws_ems/get_school_settings?username=$username&cookie=$cookie_value&ip=$ip&scid=$edt_usr";
		//echo $url;
		$myfile = fopen($url, "r")
				or die("died when doing OPEN ws_doua_ems.get_school_settings");
		$allLines = "";
		...
		$sname	= "";		
		if ($program_settings != "") {			
			$sname	= $program_settings;
		}

	
		echo "<h1>$edtSchoolTitle</h1>";
		echo "<p>&nbsp; </p>\n";
		echo "<p>$currSchoolMsg: $sname</p>\n";
		echo '<p><div id="edt_err_msg" style="color:#FF0000"></div></p>';
		echo '<form action="" method="get" name="frmEdit" id="frmEdit">';
			echo '<table width="353" align="center">';
				echo "<tr>\n";					
					echo "<td><div align=\"right\">$schoolMsg</div></td>";
					echo "<td colspan=\"2\"><input name=\"txtSchool\" type=\"text\" id=\"txtSchool\" value=\"$sname\"></td>\n";
				echo "</tr>\n";			
				echo "<tr>\n";
					echo "<td><input name=\"btnBack\" type=\"button\" id=\"btnBack\" value=\"$backBtnMsg\" onClick=\"back_page('man_school.php?user=$username')\"></td>";
					echo "<td><input name=\"btnClear\" type=\"reset\" id=\"btnClear\" value=\"$clearBtnMsg\"></td>";
					echo "<td><input name=\"btnSave\" type=\"button\" id=\"btnSave\" value=\"$saveBtnMsg\" onClick=\"edit_school()\"></td>";
				echo "</tr>\n";
			echo "</table>\n";	
